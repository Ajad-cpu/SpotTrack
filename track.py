...
def detect(opt):
    ...
    for frame_idx, (path, img, im0s, vid_cap, s) in enumerate(dataset):
        ...
        # Inference
        visualize = increment_path(Path(out) / Path(path).stem, mkdir=True) if opt.visualize else False
            ...
            if det is not None and len(det):
                ...
                if len(outputs) > 0:
                    for j, output in enumerate(outputs):
                        ...
                        # Use the confidence from the original detection if available; otherwise 0
                        conf_val = confs[j].item() if j < len(confs) else 0.0
                        label = f'{id} {names[c]} {conf_val:.2f}'
